...
def model_run(base, mode, symptom_idx, symptom_name):
    ...
    if not os.path.exists(os.path.join('binary')):
        os.mkdir(os.path.join('binary'))
    if not os.path.exists(os.path.join('binary', base)):
        os.mkdir(os.path.join('binary', base))
    if not os.path.exists(os.path.join('binary', base, symptom)):
        os.mkdir(os.path.join('binary', base, symptom))
    ...
    for k in range(30):
        # check out
        out_path = os.path.join('binary', base, str(symptom), '_'.join([str(mode), str(k)]))
        ...
        # Define model
        model = Prediction_model('binary', base).to(device)
        ...
        for e in tqdm(range(100)):
            train_loss, train_metrics = train_binary(model, optimizer, criterion, train_batch, symptom_idx, 'CE', device)
            test_loss, test_metrics = evaluate_binary(model, criterion, test_batch, symptom_idx, 'CE', device)
        ...
        # torch.save(model.state_dict(), os.path.join('binary', base, symptom, '_'.join([str(mode), str(k), '.pth'])))
        ...
        print(base, str(symptom), str(mode), str(k))
...
if __name__ == "__main__":
    ...
    print('- symptom: {}'.format(mode))
    print('====================')

    model_run(base, mode, symptom_idx, symptom_name)
